...
const buildValidationSchema = (formFields: FormField[]) => {
  return Yup.object(
    formFields.reduce((schema, field) => {
      ...
      if (field.regex) {
        validator = validator.matches(new RegExp(field.regex), `${field.name} is not valid`);
      }
      ...
      if (field.maxLength) {
        validator = validator.max(field.maxLength, `${field.name} must be at most ${field.maxLength} characters`);
      }
