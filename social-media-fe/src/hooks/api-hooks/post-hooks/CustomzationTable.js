...
function CustomzationTable() {
  ...
  const {
    ...
    isFetching: customizationsFetching,
  } = useGetAllCustomizations();
      ...
      const fetchOptions = async () => {
        const optionsData = await Promise.all(
          customizations.map(async (item) => {
            ...
            try {
              const response =
                await getOptionsViaCustomization(customization_id);
              const options = response.data.data;
              return {
                customizationId: customization_id,
                customizationName: customization,
                customizationNote:
                  !note || note === "" ? "No information" : note,
                optionsData: options,
              ...
              console.error(
                `Error fetching options for customization ${customization_id}:`,
                error,
              );
              return {
                customizationId: customization_id,
  ...
  }, [customizations, customizationsFetching, filterSelector.searchInput]);
      ...
      {customizationsLoading || customizationsFetching ? (
        ...
        <RMSTable
          ...
          frameworkComponents={{
            ...
            rule: (params) => (
              <CustomizationRules rule={params.customizationRules} />
            ),
            priceChipLabels: (params) => (
              <CustomizationChipLabels status={params.isPricing} />
            ),
          }}
        />
      )}
